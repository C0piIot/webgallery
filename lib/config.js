...
// Hosts where `http://` is acceptable: explicit loopback names, plus
// single-label hostnames (Docker / k8s service names, NetBIOS) and
// .local mDNS hosts. Public domains (anything with a dot that isn't
// .local) still require https:// to keep credentials off plaintext links.
    ...
    if (url) {
      if (url.protocol === 'http:') {
        if (!isLocalHost(url.hostname)) {
          errors.push({
            field: 'endpoint',
            message: 'http:// is only allowed for local hosts (localhost, single-label, *.local)',
          });
        }
      } else if (url.protocol !== 'https:') {
        errors.push({
          field: 'endpoint',
          message: 'must use http:// (local hosts only) or https://',
        });
      }
    }
  }
...
// --- JSON export / import (for moving config between devices). ---
//
// The exported file contains the full config including the secret
// access key in plaintext — the user is told to treat it like a
// password. We bump SCHEMA_VERSION on any breaking field change.
...
export function serializeConfigForExport(config) {
  const c = config ?? {};
  return {
...
export function parseImportedConfig(text) {
  ...
  try {
    ...
    throw new ConfigImportError('File is not valid JSON.');
  }
  if (typeof doc !== 'object' || doc === null) {
    throw new ConfigImportError('File does not contain a config object.');
  }
  if (doc.schemaVersion !== CONFIG_SCHEMA_VERSION) {
    throw new ConfigImportError(
      `Unsupported schemaVersion: ${doc.schemaVersion}. Expected ${CONFIG_SCHEMA_VERSION}.`,
    );
  }
  if (typeof doc.config !== 'object' || doc.config === null) {
    throw new ConfigImportError('File is missing a "config" object.');
  }
  return { ...doc.config };
}
