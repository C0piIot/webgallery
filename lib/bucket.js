...
export function createBucketClient(config) {
  ...
  async function put(key, body, { contentType, contentDisposition, metadata } = {}) {
  ...
  async function createMultipartUpload(key, {
    contentType, contentDisposition, metadata,
  } = {}) {
    const headers = { ...metaHeaders(metadata) };
    if (contentType) headers['content-type'] = contentType;
    if (contentDisposition) headers['content-disposition'] = contentDisposition;
  ...
  // Presigned GET URL — signature carried in the query string so the
  // browser can fetch without setting an Authorization header. Used by
  // the Remote tab gallery to render <img> thumbnails.
